...
function applyHighlight(text) {
  ...
  return text
    ...
    .map((line) => {
      ...
      return (
        escape(line.slice(0, idx)) +
        '<span class="syntax-comment">' +
        escape(line.slice(idx)) +
        '</span>'
      )
    })
    .join('\n')
...
const REMOTE_CONFIG_URL =
  'https://raw.githubusercontent.com/sansan0/TrendRadar/refs/heads/master/config/config.yaml'
const REMOTE_FREQUENCY_URL =
  'https://raw.githubusercontent.com/sansan0/TrendRadar/refs/heads/master/config/frequency_words.txt'
const REMOTE_TIMELINE_URL =
  'https://raw.githubusercontent.com/sansan0/TrendRadar/refs/heads/master/config/timeline.yaml'
const REMOTE_VERSION_URL =
  'https://raw.githubusercontent.com/sansan0/TrendRadar/refs/heads/master/version_configs'
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (savedFrequency && savedFrequency.trim()) {
    ...
    frequencyEditor.value =
      '# 在此粘贴你的 frequency_words.txt 内容...\n# 或拖拽文件到编辑器区域\n\n[GLOBAL_FILTER]\n\n[WORD_GROUPS]\n'
  ...
  if (
    savedTimeline &&
    savedTimeline.trim() &&
    savedTimeline !== INITIAL_TIMELINE
  ) {
  ...
  frequencyEditor.addEventListener('scroll', () =>
...
function handleFileDrop(e, type) {
  ...
  if (!isValid) {
    showToast(
      `请拖入 ${
        type === 'config' || type === 'timeline' ? 'YAML' : 'TXT'
      } 文件`,
      'error'
    )
    return
...
function updateSaveTimeDisplay() {
  ...
  if (configTimeEl) {
    ...
    configTimeEl.title = configTime
      ? new Date(configTime).toLocaleString('zh-CN')
      : '未保存'
    if (configLabelEl) {
      if (configTime) {
  ...
  if (frequencyTimeEl) {
    ...
    frequencyTimeEl.title = frequencyTime
      ? new Date(frequencyTime).toLocaleString('zh-CN')
      : '未保存'
    if (frequencyLabelEl) {
      if (frequencyTime) {
  ...
  if (timelineTimeEl) {
    ...
    timelineTimeEl.title = timelineTime
      ? new Date(timelineTime).toLocaleString('zh-CN')
      : '未保存'
    if (timelineLabelEl) {
      if (timelineTime) {
...
window.confirmLoadConfig = async function () {
  ...
  try {
    ...
    if (loadConfig)
      promises.push(
        fetch(REMOTE_CONFIG_URL).then((r) => ({ type: 'config', res: r }))
      )
    if (loadFrequency)
      promises.push(
        fetch(REMOTE_FREQUENCY_URL).then((r) => ({ type: 'frequency', res: r }))
      )
    if (loadTimeline)
      promises.push(
        fetch(REMOTE_TIMELINE_URL).then((r) => ({ type: 'timeline', res: r }))
      )
...
function renderModules() {
  ...
  MODULE_DEFS.forEach((mod) => {
    ...
    const header = `
            <div class="module-header px-4 py-3 flex items-center justify-between cursor-pointer" onclick="scrollToModuleInEditor('${
              mod.key
            }')">
    ...
    const body = mod.editable
      ? `<div class="module-body p-5 border-t border-gray-50 space-y-4" id="controls-${mod.key}"></div>`
      : ''
...
function renderModuleNav() {
  ...
  nav.innerHTML = MODULE_DEFS.map(
    (mod) => `
        <button onclick="scrollToModuleInEditor('${mod.key}')"
                class="module-nav-btn text-[10px] px-2 py-1 rounded ${
                  mod.editable
...
window.scrollToModuleInEditor = function (modKey) {
  ...
  // 设置光标位置
  const textBeforeTarget =
    lines.slice(0, targetLineIndex).join('\n').length +
    (targetLineIndex > 0 ? 1 : 0)
  editor.focus()
  editor.setSelectionRange(
    textBeforeTarget,
    textBeforeTarget + lines[targetLineIndex].length
  )
...
function renderControls(mod) {
  ...
  switch (mod.key) {
    ...
    case 'rss':
      ...
      html += createToggleControl(
        mod.key,
        'freshness_filter.enabled',
        '启用新鲜度过滤'
      )
      html += createNumberControl(
        mod.key,
        'freshness_filter.max_age_days',
        '最大文章年龄 (天)'
      )
      ...
      break
    case 'report':
      html = createSelectControl(mod.key, 'mode', '报告模式', [
        'current',
        'daily',
        'incremental',
      ])
      html += createSelectControl(mod.key, 'display_mode', '分组维度', [
        'keyword',
        'platform',
      ])
      html += createToggleControl(
    ...
    case 'ai_analysis':
      ...
      html += createSelectControl(mod.key, 'mode', 'AI 分析模式', [
        'follow_report',
        'daily',
        'current',
        'incremental',
...
window.switchTab = function (tab) {
  ...
  const activeClass =
    'tab-button active px-4 py-2 text-xs font-bold text-gray-300 hover:bg-[#2d2d30] transition-colors border-b-2 border-blue-500'
  const inactiveClass =
    'tab-button px-4 py-2 text-xs font-bold text-gray-500 hover:bg-[#2d2d30] transition-colors border-b-2 border-transparent'
  ...
  // 更新编辑器显示
  document
    .getElementById('yaml-editor-wrap')
    .classList.toggle('hidden', tab !== 'config')
  document
    .getElementById('frequency-editor-wrap')
    .classList.toggle('hidden', tab !== 'frequency')
  document
    .getElementById('timeline-editor-wrap')
  ...
  document
    ...
    .classList.toggle('hidden', tab !== 'config')
  document
    .getElementById('frequency-panel')
    .classList.toggle('hidden', tab !== 'frequency')
  document
  ...
  if (saveTimeConfig)
    saveTimeConfig.classList.toggle('hidden', tab !== 'config')
  if (saveTimeFrequency)
    saveTimeFrequency.classList.toggle('hidden', tab !== 'frequency')
  if (saveTimeTimeline)
    saveTimeTimeline.classList.toggle('hidden', tab !== 'timeline')
...
function parseFrequencyText(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentSection === 'global') {
      ...
      if (
        groupNameMatch &&
        !['GLOBAL_FILTER', 'WORD_GROUPS'].includes(groupNameMatch[1])
      ) {
        // 保存当前词组到缓存
        if (currentGroup) {
          relatedGroupsBuffer.push(currentGroup)
...
function buildFrequencyText(data) {
  ...
  if (data.originalText) {
    ...
    data.wordGroups.forEach((group, index) => {
      ...
      const bothAreRelatedGroups =
        group.isRelatedGroup && nextGroup && nextGroup.isRelatedGroup

      // 如果下一个词组有前置注释，不需要额外添加空行（注释中已包含空行）
      const nextHasComments =
        nextGroup &&
        nextGroup.precedingComments &&
        nextGroup.precedingComments.length > 0
...
function renderFrequencyPanel(data) {
  ...
  function renderGroupCard(group, idx) {
    ...
    // 序号标记
    const indexBadge = `<span class="text-xs bg-gray-700 text-white px-2.5 py-1 rounded-full font-bold mr-2" title="词组序号">#${
      idx + 1
    }</span>`
    ...
    const relatedGroupBadge = group.isRelatedGroup
      ? `<span class="text-[10px] bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-2 py-0.5 rounded font-bold ml-2" title="此组与相邻组相关（无空行分隔）">
                <i class="fa-solid fa-link mr-1"></i>相关组 ${
                  group.relatedGroupIndex + 1
                }/${group.relatedGroupTotal}
               </span>`
      : ''
                            ...
                            ${group.keywords
                              .map((kw) => {
                                ...
                                const escapedKw = kw
                                  ...
                                  .replace(/"/g, '&quot;')
                                return `
                                    <span class="tag-item ${getKeywordClass(
                                      kw
                                    )} relative break-all cursor-pointer" data-keyword="${escapedKw}" onclick="editKeyword(${idx}, this.dataset.keyword, this)">
                                        ${
                                          label
                                            ? `<span class="text-[9px] opacity-75 mr-1">[${label}]</span>`
                                            : ''
                                        }
                                        ${escapedKw}
  ...
  panel.innerHTML = `
                ...
                ${data.globalFilter
                  .map(
                    ...
                    <span class="tag-item ${getKeywordClass(f)}">
                        ${f}
                        <button onclick="removeGlobalFilter('${f.replace(
                          /'/g,
                          "\\'"
                        )}')">×</button>
                    </span>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-5">
            <div class="flex items-center justify-between mb-3">
                <h3 class="text-sm font-bold text-gray-700">
                    <i class="fa-solid fa-layer-group mr-2"></i>关键词组 <span class="text-xs text-gray-400 font-normal">(${
                  ...
                  .map((group, idx) => {
                    ...
                    if (idx < data.wordGroups.length - 1) {
                      return (
                        card +
                        `
                            <div class="insert-zone group/insert" data-insert-index="${
                              idx + 1
                            }">
                                <button onclick="insertWordGroupAt(${
                                  idx + 1
                                })" class="insert-button">
      ...
      container.sortableInstance = new Sortable(container, {
        ...
        onEnd: function (evt) {
          ...
          const cards = Array.from(
            container.querySelectorAll('.word-group-card')
          )
          const newOrder = cards.map((card) =>
            parseInt(card.getAttribute('data-group-index'))
          )
  ...
  setTimeout(() => {
    ...
    if (pendingWordGroupPosition === 'bottom') {
      ...
      if (cards[pendingWordGroupPosition]) {
        cards[pendingWordGroupPosition].scrollIntoView({
          behavior: 'smooth',
          block: 'center',
        })
      }
...
window.editKeyword = function (groupIndex, oldKeyword, spanElement) {
  ...
  const originalKeyword =
    group.keywords.find((kw) => kw === oldKeyword) || oldKeyword

  const input = document.createElement('input')
  input.type = 'text'
  input.value = originalKeyword
  input.className =
    'tag-input inline-block px-2 py-1 text-xs border border-blue-500 rounded'
...
- 只要这一行正则表达式，不要任何解释，不要代码块。`
  ...
  if (copySuccess) {
    ...
    prompt(
      '自动复制失败，请手动复制以下内容，然后自行打开 DeepSeek:',
      promptText
    )
    window.open('https://chat.deepseek.com/', '_blank')
  }
...
function updatePlatformsInYaml(platforms) {
  ...
  const indent = '    ' // 4 空格缩进
  const newSourcesLines = platforms
    .map((p) => `${indent}- id: "${p.id}"\n${indent}  name: "${p.name}"`)
    .join('\n')
      ...
      if (regionOrder.length > 0) {
        return regionOrder.map((key) => {
          ...
          return {
            ...
            enabled:
              regionStates[normalizedKey] !== undefined
                ? regionStates[normalizedKey]
                : false,
          }
        })
      }
...
function renderDisplayRegionsList() {
  ...
  container.innerHTML = regions
    .map(
      (r, idx) => `
        <div class="display-region-item flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2 border border-gray-200 hover:border-blue-300 transition-colors" data-key="${
          r.key
        }">
            <div class="flex items-center gap-2">
                <i class="fa-solid fa-grip-vertical text-gray-300 cursor-move"></i>
                <span class="text-xs font-medium ${
                  r.enabled ? 'text-gray-700' : 'text-gray-400'
                }">${r.label}</span>
...
function updateDisplayRegionsInYaml(regions) {
  ...
  if (regionOrderStart > 0 && regionOrderEnd > regionOrderStart) {
    ...
    // 提取原有行的注释映射
    const originalRegionOrderBlock = lines.slice(
      regionOrderStart,
      regionOrderEnd
    )
    const commentMap = {}
    ...
    lines.splice(
      regionOrderStart,
      regionOrderEnd - regionOrderStart,
      ...newRegionOrderLines
    )

    // 调整 regionsStart 和 regionsEnd
    const lineDiff =
      newRegionOrderLines.length - (regionOrderEnd - regionOrderStart)
  ...
  if (regionsStart > 0 && regionsEnd > regionsStart) {
    ...
    const newRegionsLines = regions.map((r) => {
      const comment = commentMap[r.key] || ''
      return `${indentStr}${r.key}: ${r.enabled}${
        ...
        <div class="rss-feed-item bg-gray-50 rounded-lg px-3 py-2 border border-gray-200 hover:border-blue-300 transition-colors" data-index="${idx}">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    ${
                      f.enabled === false
                        ? '<span class="text-[9px] bg-gray-200 text-gray-500 px-1 rounded">已禁用</span>'
                        : ''
                    }
                </div>
                <div class="flex items-center gap-1">
                    ...
                    <button onclick="toggleRssFeed(${idx})" class="text-gray-400 hover:text-gray-600 text-xs px-1" title="${
        f.enabled === false ? '启用' : '禁用'
      }">
                        <i class="fa-solid fa-${
                          f.enabled === false ? 'eye' : 'eye-slash'
                        }"></i>
                    </button>
            ...
            <div class="text-[10px] text-gray-400 mt-1 truncate" title="${
              f.url
            }">${f.url}</div>
        </div>
    `
...
function openRssModalWithData(feed, editIndex) {
  ...
  document.getElementById('rss-max-age').value =
    feed && feed.max_age_days !== undefined ? feed.max_age_days : ''
...
function renderStandaloneLists() {
  ...
  if (availablePlatforms.length === 0) {
    ...
    platformsContainer.innerHTML = availablePlatforms
      .map((p) => {
        ...
        return `
                <label class="flex items-center gap-2 p-1.5 rounded hover:bg-white transition-colors cursor-pointer">
                    <input type="checkbox" onchange="toggleStandaloneItem('platforms', '${
                      p.id
                    }')"
                           ${
                             isChecked ? 'checked' : ''
  ...
  if (availableRss.length === 0) {
    ...
    rssContainer.innerHTML = availableRss
      .map((f) => {
        const isChecked = standaloneConfig.rss_feeds.includes(f.id)
        return `
                <label class="flex items-center gap-2 p-1.5 rounded hover:bg-white transition-colors cursor-pointer">
                    <input type="checkbox" onchange="toggleStandaloneItem('rss_feeds', '${
                      f.id
                    }')"
                           ${
                             isChecked ? 'checked' : ''
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-[9px] text-gray-400 ml-2">${
                              f.id
                            }</span>
                        </div>
                        <div class="text-[9px] text-gray-400 truncate">${
...
function updateStandaloneConfigInYaml(type, list) {
  ...
  if (targetLineIndex !== -1) {
    ...
    lines[targetLineIndex] = `${indent}${type}: ${jsonStr}${
      comment ? ' ' + comment : ''
    }`
...
window.checkVersion = async function () {
  ...
  btn.innerHTML =
    '<i class="fa-solid fa-spinner fa-spin"></i><span>检测中...</span>'
  btn.disabled = true
...
function showVersionComparisonModal(fileName, currentVersion, latestVersion) {
  ...
  if (!currentVersion) {
    statusIcon =
      '<i class="fa-solid fa-question-circle text-gray-500 text-3xl"></i>'
    statusText = '未检测到版本信息'
            ...
            <div class="bg-gray-50 rounded-lg p-4 space-y-3 mb-4">
                ...
                <div class="flex items-center justify-between text-sm">
                    ...
                    <span class="font-mono font-bold ${
                      currentVersion ? 'text-blue-600' : 'text-gray-400'
                    }">
...
function renderAvailablePlatforms() {
  ...
  const available = PRESET_PLATFORMS.filter(
  ...
  if (available.length === 0) {
    ...
    available.forEach((p) => {
      ...
      div.className =
        'flex items-center justify-between p-3 border border-gray-100 rounded hover:bg-blue-50 cursor-pointer transition-colors group'
...
window.updateToLatest = async function () {
  ...
  const currentTab = getCurrentTab()
  const fileName =
    currentTab === 'config' ? 'config.yaml' : 'frequency_words.txt'

  if (
    !confirm(
      `确定要从 GitHub 更新 ${fileName} 到最新版本吗？\n\n你当前的自定义配置将被覆盖，建议先复制保存。`
    )
  ) {
    return
  }
  ...
  try {
    const url =
      currentTab === 'config' ? REMOTE_CONFIG_URL : REMOTE_FREQUENCY_URL
...
function toggleRssTips() {
  ...
  if (panel) {
    ...
    if (icon) {
      icon.style.transform = panel.classList.contains('hidden')
        ? 'rotate(0deg)'
        : 'rotate(180deg)'
    }
  }
...
function syncTimelineToUI() {
  ...
  allModes.forEach((name) => {
    ...
    html += `
            <div class="tl-preset-card ${
              isActive ? 'selected' : ''
            }" data-preset="${name}">
                ${
                  meta.recommend
                    ? '<div class="tl-recommend-badge">推荐</div>'
                    : ''
                }
                <div class="flex items-center gap-3 cursor-pointer" onclick="selectTimelinePreset('${name}')">
                    <div class="tl-card-icon ${meta.bg} ${
      meta.color
    }"><i class="fa-solid ${meta.icon}"></i></div>
                    <div class="flex-1 min-w-0">
                        <div class="text-sm font-bold text-gray-800 truncate tl-editable" ondblclick="event.stopPropagation();tlInlineEdit(this,'${name}','name','${escapeAttr(
      label
    )}')">${label}</div>
                        <div class="text-[10px] text-gray-500 truncate tl-editable" ondblclick="event.stopPropagation();tlInlineEdit(this,'${name}','description','${escapeAttr(
      desc
    )}')">${desc}</div>
                    </div>
                </div>
                <div class="tl-card-actions">
                    ...
                    ${
                      !isProtected
                        ? `<button onclick="event.stopPropagation();deleteTlPreset('${name}')" class="tl-card-action-btn text-red-400 hover:text-red-600" title="删除"><i class="fa-regular fa-trash-can"></i></button>`
                        : ''
                    }
                </div>
                ${
                  isActive
                    ? '<div class="absolute bottom-1 right-2 text-[9px] text-blue-500 font-bold"><i class="fa-solid fa-check-circle mr-0.5"></i>当前</div>'
                    : ''
                }
            </div>`
  })
...
function renderWeekView(config, presetName) {
  ...
  for (let h = 0; h <= 24; h += 2) {
    html += `<div class="tl-hour-marker" style="width:${100 / 12}%;${
      h === 24 ? 'text-align:right;margin-left:-1em' : ''
    }">
            ${h < 10 ? '0' : ''}${h}
        </div>`
  }
  ...
  for (let d = 1; d <= 7; d++) {
    ...
    html += `<div class="tl-week-row">
            <div class="tl-day-label ${isToday ? 'today' : ''}">${
      DAY_NAMES[d - 1]
    }</div>
    ...
    dayPeriodNames.forEach((pName) => {
      ...
      blocks.forEach((b) => {
        ...
        html += `<div class="tl-period-block ${colorClass}" style="left:${left}%;width:${width}%"
                              ...
                              onmouseenter="showTlTooltip(event, '${escapeAttr(
                                label
                              )}', '${p.start || ''}', '${
          p.end || ''
        }', ${!!merged.push}, ${!!merged.analyze}, '${
          merged.report_mode || ''
        }')"
    ...
    if (isToday) {
      ...
      html += `<div class="tl-now-line" style="left:${nowLeftPct}%" title="当前时间 ${String(
        nowTime.getHours()
      ).padStart(2, '0')}:${String(nowTime.getMinutes()).padStart(
        2,
        '0'
      )}"></div>`
    }
...
function showTlTooltip(event, name, start, end, push, analyze, mode) {
  ...
  if (!push && !analyze)
    features.push('<span style="color:#94a3b8">仅采集</span>')
  ...
  el.innerHTML = `<div style="font-weight:700;margin-bottom:2px">${name}</div>
        ...
        ${
          mode
            ? `<div style="font-size:10px;color:#9ca3af;margin-top:2px">模式: ${mode}</div>`
            : ''
        }`
  ...
  if (elRect.right > window.innerWidth - 4)
    el.style.left = window.innerWidth - el.offsetWidth - 4 + 'px'
  if (elRect.top < 4) {
...
function renderPeriodDetails(config, presetName) {
      ...
      html += `<div class="tl-period-card" id="tl-period-${key}">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-bold text-gray-800 tl-editable" ondblclick="tlInlineEditPeriod(this,'${presetName}','${key}','${escapeAttr(
        p.name || key
      )}')">${p.name || key}</span>
                    ...
                    ${
                      pList.length > 0
                        ? pList
                            ...
                            .join('')
                        : '<span class="text-[10px] text-gray-400">空 (全天走 default)</span>'
                    }
                    ${
  ...
  dayPlanKeys.forEach((k, idx) => {
    planColorMap[k] = planColors[idx % planColors.length]
  })
...
function renderBehaviorToggles(cfg, presetName, periodKey) {
        ...
        <select class="text-[10px] border border-gray-200 rounded px-1 py-0.5 bg-white"
            ...
            ${aiModes
              .map(
                (m) =>
                  `<option value="${m}" ${
                    (cfg.ai_mode || 'follow_report') === m ? 'selected' : ''
                  }>${m}</option>`
              )
              .join('')}
        </select>
  ...
  html += `<label class="flex items-center gap-1 text-[10px] ${
    onceAnalyze ? 'text-blue-600' : 'text-gray-400'
  ...
  html += `<label class="flex items-center gap-1 text-[10px] ${
    oncePush ? 'text-blue-600' : 'text-gray-400'
  }" style="cursor:pointer">
...
window.onTlWeekMap = function (presetName, dayNum, value) {
  ...
  for (let i = sectionStart + 1; i < lines.length; i++) {
    const line = lines[i]
    if (line.trim() === '' || line.trim().startsWith('#')) continue
    if (line.search(/\S/) <= sectionIndent) {
      sectionEnd = i
      break
    }
  }
  ...
  // 找 week_map: 行
  const weekMapLine = findChildKey(
    lines,
    sectionStart,
    sectionEnd,
    sectionIndent,
...
window.openTlNewPresetModal = function () {
  ...
  if (data?.custom) {
    sel.innerHTML += `<option value="custom">${
      data.custom.name || '自定义'
    } (custom)</option>`
  }
  // 清空输入
...
window.confirmTlNewPreset = function () {
  ...
  // 验证
  if (!key) {
    showToast('请输入模式标识 (key)', 'error')
    return
  }
  if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(key)) {
    showToast('key 仅支持英文、数字和下划线，且不能以数字开头', 'error')
    return
  }
  if (!name) {
  ...
  const data = parseTimelineData()
  if (data?.presets?.[key]) {
    showToast(`预设「${key}」已存在`, 'error')
    return
  }
  if (key === 'custom') {
    showToast('不能使用 "custom" 作为预设名', 'error')
    return
  }
  ...
  if (template && data) {
    ...
    if (src) {
      block = buildPresetYamlBlock(key, {
        ...src,
        name: name,
...
window.confirmTlNewPeriod = function () {
  ...
  if (presetCfg?.periods?.[key]) {
    showToast(`时间段「${key}」已存在`, 'error')
    return
  }
  ...
  if (periodsLine < 0) {
    showToast('未找到 periods 配置段', 'error')
    return
  }
...
window.deleteTlPeriod = function (presetName, periodKey) {
  ...
  if (refs.length > 0) {
    ...
    if (updatedSection)
      removePeriodFromDayPlans(lines, updatedSection, periodKey)
  }
...
window.duplicateTlPeriod = function (presetName, periodKey) {
  ...
  let i = 2
  while (config.periods[newKey]) {
    newKey = periodKey + '_copy' + i
    i++
  }
...
window.addTlDayPlan = function (presetName) {
  ...
  const dayPlansLine = findChildKey(
    lines,
    sectionInfo.start,
    sectionInfo.end,
    sectionInfo.indent,
  ...
  lines.splice(dpEnd, 0, `${indent}${planKey}:`, `${subIndent}periods: []`)
...
window.addPeriodToDayPlan = function (presetName, planKey, periodKey) {
  ...
  if (periodsContent === 'periods: []' || periodsContent === 'periods:[]') {
    ...
    const inlineMatch = lines[periodsLine].match(
      /^(\s*periods:\s*)\[([^\]]*)\]/
    )
    if (inlineMatch) {
      ...
      const listEnd = findBlockEnd(
        lines,
        periodsLine,
        lines[periodsLine].search(/\S/),
        planEnd
      )
...
window.tlWeekMapQuick = function (presetName, mode) {
  ...
  if (dayPlanKeys.length === 0) {
    showToast('没有可用的日计划', 'error')
    return
  }
  ...
  if (mode === 'all_same') {
    ...
    if (dayPlanKeys.length < 2) {
      showToast('需要至少两个日计划来分离工作日/周末', 'warning')
      return
    }
 ...
 */
function removePeriodFromDayPlanInLines(
  lines,
  sectionInfo,
  planKey,
  periodKey
...
window.tlInlineEdit = function (el, presetName, field, currentValue) {
  ...
  input.className = `tl-inline-input ${
    isName ? 'text-sm font-bold' : 'text-[10px]'
  }`
...
window.onTlBarClick = function (event, presetName, dayNum) {
  ...
  const el = document.createElement('div')
  el.className = 'tl-tooltip'
  el.innerHTML = `<div style="font-weight:700;margin-bottom:2px">${
    DAY_NAMES[dayNum - 1]
  }</div>
  ...
  setTimeout(() => {
    if (tlTooltipEl === el) hideTlTooltip()
  }, 2000)
}
